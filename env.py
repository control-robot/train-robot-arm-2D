...
class ArmEnv(object):
    ...
    def step(self, action):
        ...
        elif (self.goal['x'] - self.goal['l']/2 < finger[0] < self.goal['x'] + self.goal['l']/2
        ) and (self.goal['y'] - self.goal['l']/2 + 100 < finger[1] < self.goal['y'] + self.goal['l']/2 + 100):
                done = False
                r = -1.
        elif (self.goal['x'] - self.goal['l']/2 < finger[0] < self.goal['x'] + self.goal['l']/2
        ) and (self.goal['y'] - self.goal['l']/2 + 200 < finger[1] < self.goal['y'] + self.goal['l']/2 + 200):
                done = False
                r = -2
        else:
            self.on_goal = 0
...
class Viewer(pyglet.window.Window):
    ...
    def __init__(self, arm_info, goal):
        ...
        self.goal_info = goal
        self.center_coord = np.array([260, 200])
        ...
        self.goal3 = self.batch.add(
            4, pyglet.gl.GL_QUADS, None,    # 4 corners
            ('v2f', [goal['x'] - goal['l'] / 2, goal['y'] - goal['l'] / 2 + 200,                # location
                     goal['x'] - goal['l'] / 2, goal['y'] + goal['l'] / 2 + 200,
                     goal['x'] + goal['l'] / 2, goal['y'] + goal['l'] / 2 + 200,
                     goal['x'] + goal['l'] / 2, goal['y'] - goal['l'] / 2 + 200]),
            ('c3B', (255, 0, 0) * 4))    # color
        self.goal2 = self.batch.add(
            4, pyglet.gl.GL_QUADS, None,    # 4 corners
            ('v2f', [goal['x'] - goal['l'] / 2, goal['y'] - goal['l'] / 2 + 100,                # location
                     goal['x'] - goal['l'] / 2, goal['y'] + goal['l'] / 2 + 100,
                     goal['x'] + goal['l'] / 2, goal['y'] + goal['l'] / 2 + 100,
                     goal['x'] + goal['l'] / 2, goal['y'] - goal['l'] / 2 + 100]),
            ('c3B', (0, 0, 0) * 4))    # color
